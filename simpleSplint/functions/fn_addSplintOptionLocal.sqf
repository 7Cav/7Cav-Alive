...
private _can_splint = {
	//Get CBA settings
	private _medic_requirement = missionNamespace getVariable ["simpleSplint_medic_requirement", 0];
	private _require_splint = missionNamespace getVariable ["simpleSplint_require_item", true];
	private _consume_splint = missionNamespace getVariable ["simpleSplint_consume_item", true];
	private _require_stable = missionNamespace getVariable ["simpleSplint_require_stable", true];
	private _require_splint_class = missionNamespace getVariable ["simpleSplint_require_item_class", "ACE_personalAidKit"];
	private _consume_splint_class = missionNamespace getVariable ["simpleSplint_consume_item_class", "ACE_personalAidKit"];
...
private _splint_legs = {
	["HitLegs", [4,5], _player, _target] call simpleSplint_fnc_splintLimb;
};

private _splint_arms = {
	["HitHands", [2,3], _player, _target] call simpleSplint_fnc_splintLimb;
};

private _action = ["SplintParent", "Apply SAM Splint", "simpleSplint\ui\splint_action.paa", {true}, _can_splint] call ace_interact_menu_fnc_createAction;
